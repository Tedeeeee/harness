...
sufficient 판정 시, 사용자가 승인한 바로 그 트랜잭션에서 다음 두 파일을 같이 갱신해야 합니다. 하나만 성공하고 다른 하나가 실패하면 요구사항은 `assessed` 상태로 남기고 `confirmed`로 올리지 않습니다.

1. 요구사항 문서 Metadata:
   - `Status: confirmed`
   - `Confirmed By`: 승인한 사용자 식별 또는 세션 주체
   - `Confirmed At`: ISO 8601 날짜
2. `docs/plans/planning-state.md`:
   - `Requirements Confirmed: yes`
   - `Stage Status`의 `requirements` 행을 `complete`로
   - `Linked Planning State` 경로가 요구사항 Metadata와 일치하는지 확인

insufficient 판정 시에는 둘 다 바꾸지 않습니다. planning-state의 `requirements` 행만 `blocked`로 전환하고, 요구사항 문서는 `Status: assessed`로 둡니다 (confirmed로 올리지 않음).
...
- 요구사항 Metadata를 `Status: confirmed`로 전환하고 `Confirmed By`, `Confirmed At`을 기록합니다
- `docs/plans/planning-state.md`의 requirements 단계를 `complete`로 갱신하고 `Requirements Confirmed: yes`를 기록합니다
- 두 파일 갱신이 모두 성공했을 때만 sufficient로 보고합니다
...
- 선택한 requirements 파일을 식별합니다
- 요구사항 Metadata를 `Status: assessed`로 전환합니다 (confirmed 금지)
- `docs/plans/planning-state.md`의 requirements 단계를 `blocked`로 갱신하고 `Requirements Confirmed: no`를 유지합니다
- 아직 충분하지 않다고 말합니다
- 정확히 하나의 차단 질문만 합니다
