...
4. 각 step마다 아래를 정의합니다:
   ...
   - `screen_ids`: visual-source-analysis의 Screen ID 리스트 (아래 Visual 할당 규칙 참조)
   - `visual_scope`: `strict | approximate | loose | not-applicable`
5. step 문서를 `docs/plans/steps/` 아래에 씁니다

## Visual 할당 규칙

`docs/visual-analysis/visual-source-analysis.md`가 존재하면:

- Screen Inventory에서 `Inclusion Status: included`인 모든 screen ID가 최소 하나의 step `screen_ids`에 반드시 등장해야 한다
- 한 screen이 여러 step에 걸쳐 구현될 수 있으면 해당하는 모든 step에 ID를 중복 기록한다
- `screen_ids`에 등장하지 않는 in-scope screen이 하나라도 있으면 이 스킬은 blocker를 발생시키고 작업을 멈춘다
- `visual_scope`는 screen 성격에 따라 선택한다 (strict = 픽셀 가까이, approximate = 레이아웃 유지, loose = 방향성만)

`docs/visual-analysis/visual-source-analysis.md`가 없으면:

- 모든 step의 `screen_ids`는 `[]`로 둔다
- `visual_scope`는 `not-applicable`로 둔다

`visual_scope`의 기본 수준이 `memory/project-memory.md`의 `visual fidelity 기본 수준 [feedback]` 항목에 기록되어 있으면 그 값을 default로 적용한다. interview에서 해당 값이 confirmed로 뒤집혔다면 interview가 우선. memory를 참조한 경우 `memory-read` trace를 기록한다.
...
- visual-source-analysis가 존재하는데 included screen 중 어떤 step에도 할당되지 않은 것이 있을 때
