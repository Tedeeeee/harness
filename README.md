...
3. 입력이 PDF, 프로토타입, 와이어프레임 같은 시각 자료이거나 HTML/JSX/CSS 기반 프로토타입 코드라면 먼저 `visual analysis`를 만듭니다.
...
PDF, 이미지, 프로토타입처럼 시각 자료가 source of truth일 때뿐 아니라, Claude Design 같은 프로토타입 코드 산출물이 입력일 때도 먼저 분석 문서를 만듭니다.

이 단계의 목적은:

- 보이는 화면 수를 먼저 고정하고
- 라우트, 상태, 인터랙션, 데이터 개념을 추출하고
- 숨김, 이동, 링크, 텍스트 변경 지시를 기록하고
- 제외 화면이 있다면 이유를 남기고
- 데모 구현과 제품 의도를 분리하고
- 애매하면 질문 1회로 멈추는 것

즉, 입력 자산을 곧바로 requirements로 바꾸지 않고 한 번 분석 문서로 고정합니다. 프로토타입 코드는 최종 구현 계약이 아니라 `요구사항 초안 입력`으로 취급합니다.
...
- 역할: PDF, 프로토타입, 이미지, 프로토타입 코드 입력을 requirements 이전에 화면/행동 inventory로 정리한 문서
