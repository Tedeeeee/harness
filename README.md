...
- 목적: planning 단계별 진행 상황을 명시적으로 추적
- 위치: `docs/plans/planning-state.md`
- 규칙: 파일 존재 여부 추론이 아닌 명시적 상태값으로 라우팅 판단
...
`planning-state.md`는 planning 단계의 상태판입니다.

- `Stage Status` 표의 `Status`는 아래 고정값만 사용합니다
  - `not-assessed` / `not-started`
  - `in-progress`
  - `complete`
  - `blocked`
- `Output Document` 열에는 실제 파일명만 적습니다

문서 관계는 아래와 같습니다.

```mermaid
flowchart LR
    A["Requirements"] --> B["Planning State"]
    B --> C["Technical Approach"]
    B --> D["Interview Decisions"]
    C --> E["Master Plan"]
    D --> E
    E --> F["Step Docs"]
    F --> G["Implementation State"]
    G --> H["Verification Docs"]
    F --> E
```
...
- `.claude/settings.json`
  - Claude Code 이벤트를 Python runtime hook에 연결하는 wiring
