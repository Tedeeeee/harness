...
Next.js App Router 기반 풀스택 앱이다. SQLite를 better-sqlite3로 직접 접근하고, Tailwind CSS로 반응형 UI를 구성한다. 서버 사이드에서 DB를 다루고, 클라이언트는 fetch로 API route를 호출한다.
...
- better-sqlite3는 네이티브 모듈이라 Windows에서 빌드 도구(node-gyp)가 필요할 수 있다
- SQLite 파일 경로가 Next.js dev 모드에서 핫리로드 시 잠길 수 있다
